...
function ask(system,user){
  var key=apiKey();if(!key){setKey();key=apiKey();if(!key)return Promise.reject(new Error('API anahtarı yok'));}
  var url='https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent';
  return fetch(url,{method:'POST',headers:{'Content-Type':'application/json','x-goog-api-key':key},body:JSON.stringify({systemInstruction:{parts:[{text:system}]},contents:[{role:'user',parts:[{text:user}]}],generationConfig:{maxOutputTokens:450,temperature:.7}})})
  .then(function(r){if(!r.ok)throw new Error('AI bağlantısı kurulamadı');return r.json();})
  .then(function(d){var parts=((((d.candidates||[])[0]||{}).content||{}).parts||[]);return parts.map(function(p){return p.text||'';}).join('').trim();});
}
