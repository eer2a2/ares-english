...
var activeRecognition=null, activeVoiceButton=null, activeVoiceTarget=null, manualStop=false;
...
function continueAfterSpeech(target){
  var map={coachText:'coachSend',roleText:'roleSend',abroadText:'abroadSend',quickText:'quickEval',surpriseText:'surpriseEval'};
  var id=map[target], b=id&&el(id); if(b) setTimeout(function(){b.click();},180);
}
function voice(target,button){
  ...
  manualStop=false; activeVoiceTarget=target; activeVoiceButton=button||null;
  ...
  r.lang='en-US'; r.interimResults=true; r.continuous=true; r.maxAlternatives=5;
  var finalText='';
  r.onresult=function(e){
    var interim='';
    for(var i=e.resultIndex;i<e.results.length;i++){
      var txt=e.results[i][0].transcript;
      if(e.results[i].isFinal) finalText+=(finalText?' ':'')+txt.trim(); else interim+=(interim?' ':'')+txt.trim();
    }
  ...
  r.onend=function(){
    var stopped=manualStop, tgt=activeVoiceTarget;
    activeRecognition=null; activeVoiceTarget=null; restoreVoiceButton(); activeVoiceButton=null;
    if(stopped && box.value.trim()) continueAfterSpeech(tgt);
    manualStop=false;
  };
  try{r.start();}catch(e){activeRecognition=null;restoreVoiceButton();activeVoiceButton=null;}
}
...
function sendRole(kind,scenario,text,feedbackId){
  ...
  ask(sys,text).then(function(z){var clean=cleanAIText(z)||'Yanıt alınamadı.';feedback(feedbackId,clean);lastAresSpoken=clean;speakEnglish(clean);}).catch(function(e){feedback(feedbackId,e.message);});
}
...
if(el('roleSend'))el('roleSend').onclick=function(){var t=el('roleText').value.trim();if(!t)return feedback('roleFeedback','Önce bir cevap söyle veya yaz.');sendRole('role',roleScenario,t,'roleFeedback');el('roleText').value='';};
...
if(el('abroadSend'))el('abroadSend').onclick=function(){var t=el('abroadText').value.trim();if(!t)return feedback('abroadFeedback','Önce bir cevap söyle veya yaz.');sendRole('abroad',abroadScenario,t,'abroadFeedback');el('abroadText').value='';};
...
if(el('coachSend'))el('coachSend').onclick=function(){var t=el('coachText').value.trim();if(!t)return;var c=el('coachChat');c.innerHTML+='<div class="msg me">'+esc(t)+'</div>';el('coachText').value='';touch();ask('You are Ares, a speaking-first coach for an A2-B1 Turkish learner. Keep the learner speaking. Ignore tiny errors. Correct at most one important error briefly, then ask exactly one natural follow-up question. Short English response.',t).then(function(z){var clean=cleanAIText(z)||z;c.innerHTML+='<div class="msg ai">'+esc(clean)+'</div>';c.scrollTop=c.scrollHeight;lastAresSpoken=clean;speakEnglish(clean);}).catch(function(err){feedback('coachFeedback',err.message);});};
...
if(el('quickEval'))el('quickEval').onclick=function(){var t=el('quickText').value.trim();if(!t)return;touch();ask('You are Ares in Quick Response mode for a Turkish A2-B1 learner. Fluency first. Reply in short English: briefly acknowledge the answer, then ask exactly one natural follow-up question. Do not explain grammar unless a major error blocks meaning. No markdown.',el('quickQ').textContent+'\nLearner answer: '+t).then(function(z){var clean=cleanAIText(z)||z;feedback('quickFeedback',clean);lastAresSpoken=clean;speakEnglish(clean);el('quickQ').textContent=clean;el('quickText').value='';}).catch(function(e){feedback('quickFeedback',e.message);});};
...
if(el('surpriseEval'))el('surpriseEval').onclick=function(){var t=el('surpriseText').value.trim();if(!t)return;touch();ask('You are the other person in a spontaneous real-life English conversation with a Turkish A2-B1 learner. Stay in character. Reply in 1-3 short natural English sentences and ask exactly one follow-up question. Keep the conversation going. No grammar explanation and no markdown.',el('surpriseQ').textContent+'\nLearner answer: '+t).then(function(z){var clean=cleanAIText(z)||z;feedback('surpriseFeedback',clean);lastAresSpoken=clean;speakEnglish(clean);el('surpriseQ').textContent=clean;el('surpriseText').value='';}).catch(function(e){feedback('surpriseFeedback',e.message);});};
